...
export interface RNElementText {
  ...
  readonly source: RNElementTextSource;
  readonly sourceType: RNElementTextSourceType;
  readonly potentiallySensitive: boolean;
}

export interface RNEventValue {
  readonly value: unknown;
  readonly source: RNEventValueSource;
  readonly sourceType: RNEventValueSourceType;
  readonly potentiallySensitive: boolean;
}
    ...
    if (event != null && typeof event === 'object') {
      ...
      if (nativeEvent != null && typeof nativeEvent === 'object') {
        const extracted = createEventValue(
          (nativeEvent as { text?: unknown }).text,
          'native_event_text',
          'user_input'
        );
