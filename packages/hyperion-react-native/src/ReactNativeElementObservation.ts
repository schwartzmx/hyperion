...
const originalFunctions = new WeakMap<JSXRuntimeFunction, JSXRuntimeFunction>();
...
export function createObservedJSXFunction<T extends JSXRuntimeFunction>(
  ...
  const wrapped = function (this: unknown, type: unknown, props: unknown) {
    ...
    let descriptor: ElementInstrumenterDescriptor | null = null;
    try {
      descriptor =
        instrumenter?.(
          type,
  ...
  originalFunctions.set(wrapped, originalFunctions.get(original) ?? original);
...
function installObservedRuntimeFunction(
  ...
  return originalFunctions.get(originalFunction) ?? originalFunction;
}

export function resetElementObservationForTests(): void {
  elementObservationEnabled = false;
